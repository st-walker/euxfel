...
from ocelot.cpbd.io import is_an_mpi_process, load_particle_array, save_particle_array2npz
...
SIZE = MPI_COMM.Get_size()  # Number of cores
...
assert is_an_mpi_process(), "Not an MPI process!"

print(os.getcwd())
job_dir = Path("./mpi-dir")


def load_config():
    rf_configs = [toml.load(f) for f in (job_dir / "rf").glob("rf-*.toml")]
    nconfigs = len(rf_configs)
    if SIZE != nconfigs:
        raise RuntimeError(f"Number of cores must equal number of configs: {SIZE=}, {nconfigs=}")
    return rf_configs[RANK]
...
this_rank_outdir = Path(f"{job_dir}/mpi-rank-{RANK}")
print(this_rank_outdir)
this_rank_outdir.mkdir(exist_ok=True)
section_lat = SectionLattice(sequence=all_sections, tws0=tws0, data_dir=str(this_rank_outdir))
...
config = {
    ...
    T20LUXE: {"match": match_exec, "SC": False, "CSR": CSR_exec},
    CL3LUXE: {"match": match_exec, "SC": False, "CSR": CSR_exec},
}


# tracking
p_array = load_particle_array(str(job_dir / "particles" / in_file))
p_array = p_array.thin_out(nth=200)
...
if T20LUXE in all_sections:
    ...
    save_particle_array2npz(this_rank_outdir / "particles" / "luxe_ip.npz", parray_ip)
